    ...
    <div className="space-y-6">
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-800 gap-6">
        <button
          ...
          className={`pb-3 flex items-center gap-2 text-sm font-semibold border-b-2 transition-colors duration-150 ${
            activeTab === 'users'
            ...
            <form onSubmit={handleCreateUser} className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-slate-900/50 border border-slate-800 text-slate-100"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-slate-900/50 border border-slate-800 text-slate-100"
                  placeholder="password (leave blank for invite)"
                />
              </div>
              <div>
              ...
              <div className="md:col-span-4 flex gap-2">
                <button type="submit" className="btn-primary px-4 py-2 rounded-lg bg-blue-600">
                  Create User
                </button>
              </div>
            </form>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                  {usersList.map((targetUser) => (
                    <tr key={targetUser._id || targetUser.id} className="hover:bg-slate-50/40 dark:hover:bg-slate-900/10 transition-colors">
                      ...
                      <td className="py-4 px-6 text-slate-500 dark:text-slate-400">{targetUser.email}</td>
                      <td className="py-4 px-6">
                        <select
                          value={targetUser.role}
                          onChange={(e) => handleRoleChange(targetUser._id || targetUser.id, e.target.value)}
                          disabled={(targetUser._id || targetUser.id) === (user._id || user.id)}
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-xs font-medium">
                  {auditLogs.map((log) => (
                    <tr key={log._id} className="hover:bg-slate-50/40 dark:hover:bg-slate-900/10 transition-colors">
                      <td className="py-3.5 px-5 text-slate-400">{new Date(log.timestamp).toLocaleString()}</td>
                      ...
                      <td className="py-3.5 px-5 text-slate-500">{log.resource}</td>
                      <td className="py-3.5 px-5">
                        {log.userId ? (
                      ...
                      <td className="py-3.5 px-5 text-slate-400 font-mono">{log.ipAddress || '127.0.0.1'}</td>
      ...
      {inspectLog && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-lg rounded-2xl shadow-2xl p-6 relative">
            <button onClick={() => setInspectLog(null)} className="absolute right-4 top-4 text-slate-400 hover:text-slate-200">
              Close
            </button>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-2 text-xs font-semibold py-2 border-b border-slate-100 dark:border-slate-800">
                <div>
                  Action: <span className="text-slate-400">{inspectLog.action}</span>
                </div>
                <div>
                  Resource: <span className="text-slate-400">{inspectLog.resource}</span>
                </div>
                <div>
                  IP Address: <span className="text-slate-400">{inspectLog.ipAddress}</span>
                </div>
                <div>
                  User: <span className="text-slate-400">{inspectLog.userId ? inspectLog.userId.name : 'Guest'}</span>
                </div>
              </div>
